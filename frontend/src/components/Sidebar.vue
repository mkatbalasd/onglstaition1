<template>
  <aside class="bg-gray-100 dark:bg-gray-800 h-full p-4">
    <nav class="space-y-2">
      <RouterLink
        to="/"
        class="flex items-center space-x-2 rtl:space-x-reverse p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-700"
      >
        <Home class="w-4 h-4 text-gray-500 dark:text-gray-400" />
        <span class="text-gray-700 dark:text-gray-100">Dashboard</span>
      </RouterLink>
      <RouterLink
        to="/driver-cards"
        class="flex items-center space-x-2 rtl:space-x-reverse p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-700"
      >
        <IdCard class="w-4 h-4 text-gray-500 dark:text-gray-400" />
        <span class="text-gray-700 dark:text-gray-100">Driver Cards</span>
      </RouterLink>
      <RouterLink
        to="/operating-cards"
        class="flex items-center space-x-2 rtl:space-x-reverse p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-700"
      >
        <SquareKanban class="w-4 h-4 text-gray-500 dark:text-gray-400" />
        <span class="text-gray-700 dark:text-gray-100">Operating Cards</span>
      </RouterLink>
      <RouterLink
        to="/facilities"
        class="flex items-center space-x-2 rtl:space-x-reverse p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-700"
      >
        <Building2 class="w-4 h-4 text-gray-500 dark:text-gray-400" />
        <span class="text-gray-700 dark:text-gray-100">Facilities</span>
      </RouterLink>
      <RouterLink
        to="/drivers"
        class="flex items-center space-x-2 rtl:space-x-reverse p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-700"
      >
        <Users class="w-4 h-4 text-gray-500 dark:text-gray-400" />
        <span class="text-gray-700 dark:text-gray-100">Drivers</span>
      </RouterLink>
      <RouterLink
        to="/vehicles"
        class="flex items-center space-x-2 rtl:space-x-reverse p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-700"
      >
        <Truck class="w-4 h-4 text-gray-500 dark:text-gray-400" />
        <span class="text-gray-700 dark:text-gray-100">Vehicles</span>
      </RouterLink>
    </nav>
  </aside>
</template>

<script setup>
import { Home, IdCard, SquareKanban, Building2, Users, Truck } from 'lucide-vue-next'
import { RouterLink } from 'vue-router'
</script>

<style scoped>
</style>
